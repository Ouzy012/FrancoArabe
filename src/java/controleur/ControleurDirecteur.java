...
public class ControleurDirecteur extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equals("ajoutEleve")) {
            ...
            Personne personne = new Personne(login, nom, prenom, adresse, tel, motDePasse, 1, "Surveillant");
            boolean resultat = daoDirecteur.ajouterSurv(personne);
            if (resultat) {
                String msg = "ajout";
...
//            Collections.reverse(eleves);
            ...
            if (resultat) {
                profs = daoProf.listerProf(annee);
            ...
            String login = request.getParameter("login");
            Utilisateur usr = daoProf.listerUnSurv(login);
            ...
            String login = request.getParameter("login");
            daoDirecteur.supprimerSurv(login);
            ArrayList<Utilisateur> users = daoProf.listerSurv();
            request.setAttribute("users", users);
            rd = request.getRequestDispatcher("directeur/listeSurv.jsp");
            ...
            daoDirecteur.updateMatiereFr(nomFr.toUpperCase() + ":Francais", nomFrancaisOld);
            ...
            int index = nomEnFrancais.indexOf(":");
            String matFrOnly = nomEnFrancais.substring(0, index);
            String langue = nomEnFrancais.substring(index, nomEnFrancais.length());
            System.out.println("nom Arabe " + matFrOnly + " et LAngue " + langue);
            daoDirecteur.updateMatiere(nomEnAr, matFrOnly.toUpperCase() + langue, nomEnFrancaisOld);
            ArrayList<Arabe> arabe = daoProf.listerMatEnArabe();
            request.setAttribute("arabe", arabe);
            rd = request.getRequestDispatcher("directeur/listeMatiere.jsp");
            ...
            if (nouveauMdp.equals(confirmerMdp)) {
                System.out.println("login "+login+" pmdp "+ancienMdp);
                if ((daoEleve.determinerMotDePasse(login,ancienMdp) == true)) {
                    daoDirecteur.changePasswdSurv(login, nouveauMdp, prenom, nom, adresse);
        ...
        //////////////////////Emploi du temps/////////////////////////
        else if (action.equals("accueilEDT")) {
